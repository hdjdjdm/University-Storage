...
/* To compile:  cl6x -qko3 c8_unif.c unif8.sa a8_unif.asm -z -l lnk.cmd     */
/*                   -l rts6201.lib -o c8_unif.out                          */
/****************************************************************************/
#include <stdio.h>
#include <time.h>
...
int unrolled_if_then_stub(const short a[], int codeword, int mask, short theta)
{
    int i;
...
clock_t time_stub(int *i)
{
    ...
    t_start = clock();
    *i      = unrolled_if_then_stub(x, code, msk, th);
    t_stop  = clock();
